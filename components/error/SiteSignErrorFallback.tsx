...
interface SiteSignErrorFallbackProps {
  onRetry?: () => void;
  error?: Error | null;
}

export function SiteSignErrorFallback({ onRetry, error }: SiteSignErrorFallbackProps) {
  const handleRetry = () => {
    if (onRetry) {
      onRetry();
    } else {
    ...
    <div className="flex flex-1 items-center justify-center p-8 min-h-[500px]">
      <div className="w-full max-w-md rounded-2xl border border-amber-200 bg-white p-8 shadow-sm">
        ...
        <h2 className="text-lg font-bold text-slate-800 text-center">
          SiteSign temporarily unavailable
        </h2>

        <p className="mt-2 text-sm text-slate-500 text-center">
          We couldn&apos;t load the site sign-in records. This might be due to a network issue or temporary service disruption.
        </p>
        ...
        <div className="mt-6 pt-4 border-t border-slate-100">
          <div className="flex items-start gap-2 text-xs text-slate-500">
            ...
            <p>
              Sign-in data is safely stored. Workers can continue signing in via QR code even when the dashboard is experiencing issues.
            </p>
          </div>
        </div>
      </div>
