      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <Link href="/login" className="hidden sm:inline text-sm font-bold text-slate-600 transition-colors hover:text-slate-900">
              Log in
            </Link>
            <Link href="/login?signup=1" className="rounded-xl bg-slate-950 px-3 sm:px-5 py-2.5 text-sm font-bold text-amber-200 shadow-sm transition-colors hover:bg-black">
              Create account
            </Link>
            <button
              type="button"
        ...
        {mobileOpen && (
          <nav className="md:hidden pb-4">
            <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-3 space-y-1">
              <Link href="/tools/site-sign-in" onClick={closeMenu} className="block rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">SiteSign</Link>
              <Link href="/tools/planner" onClick={closeMenu} className="block rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">SitePlan</Link>
              <Link href="/tools" onClick={closeMenu} className="block rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Workspace Apps</Link>
              <Link href="/about" onClick={closeMenu} className="block rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">About</Link>
              <Link href="/contact" onClick={closeMenu} className="block rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Contact</Link>
              <Link href="/login" onClick={closeMenu} className="block rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Log in</Link>
            </div>
          </nav>
        )}
      </div>
