...
function SidebarContent({ pathname, activeCompany, onNavigate }: SidebarContentProps) {
  const primaryLiveModules = MODULES.filter((m) => m.status === "live" && m.id === "site-sign-in");
  ...
  return (
    <>
      ...
      <div className="flex-1 py-6 px-4 space-y-8">
        ...
        <div>
          <h3 className="px-3 text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Primary Module</h3>
          <ul className="space-y-1">
            {primaryLiveModules.map((m) => {
        ...
        <div>
          <h3 className="px-3 text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Secondary Tools</h3>
          <ul className="space-y-1">
            <li>
              <Link
              ...
              >
                <div className="opacity-60">{getIcon("list-checks", "h-4 w-4")}</div>
                SitePlan
              </Link>
            </li>
          </ul>
