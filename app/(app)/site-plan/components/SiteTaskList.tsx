...
export function SiteTaskList({ tasks, delayLogs, onTaskSelect, projectId }: SiteTaskListProps) {
  ...
  const delayCountByTaskId = useMemo(() => {
    ...
    delayLogs.forEach((log) => {
      counts.set(log.task_id, (counts.get(log.task_id) ?? 0) + 1);
    });
    return counts;
  }, [delayLogs]);
                    ...
                    {group.children.map((task) => (
                      ...
                      >
                        ...
                        <div className="mt-1 flex items-center gap-2">
                          <p className="min-w-0 flex-1 truncate text-xs text-slate-400">{fmtDate(task.start_date)} → {fmtDate(task.end_date)}</p>
                          <span className="rounded-full bg-blue-50 px-2 py-0.5 text-xs font-semibold text-blue-700">{task.progress}%</span>
                        </div>
                        {(delayCountByTaskId.get(task.id) ?? 0) > 0 ? (
                          <span className="mt-1 w-fit rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-700">
                            ⚠ {delayCountByTaskId.get(task.id)} delay{(delayCountByTaskId.get(task.id) ?? 0) > 1 ? "s" : ""}
                          </span>
                        ) : null}
                      </button>
                    ))}
