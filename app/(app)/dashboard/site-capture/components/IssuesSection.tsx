  ...
  async function handleImproveWithAI() {
    ...
    try {
      ...
      const res = await fetch("/api/diary-improve-text", {
        ...
        body: JSON.stringify({ 
          text: issueForm.description,
          context: `Issue - ${issueForm.type}`
        }),
      });
      ...
      setIssueForm((prev) => ({ ...prev, description: data.text }));
      
      toast.success("Text improved with AI");
    } catch (err) {
      console.error("[IssuesSection] AI improvement error:", err);
            ...
            <form onSubmit={handleAddIssue} className="mt-4 space-y-3">
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <button
                  ...
                  disabled={improving || !issueForm.description.trim()}
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl font-medium transition-colors ${
                    improving
