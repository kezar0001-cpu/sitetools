...
/** Check OLE Compound Document signature — indicates a binary .mpp file */
function isBinaryMpp(buffer: ArrayBuffer): boolean {
  const bytes = new Uint8Array(buffer.slice(0, 4));
  return bytes[0] === 0xd0 && bytes[1] === 0xcf && bytes[2] === 0x11 && bytes[3] === 0xe0;
}

/** Send binary .mpp to the server-side parser */
async function parseMppViaApi(file: File): Promise<ImportResult> {
  const fd = new FormData();
  fd.append("file", file);
  const res = await fetch("/api/planner/parse-mpp", { method: "POST", body: fd });
  if (!res.ok) {
...
export function PlannerImportDialog({ onImport, onClose }: Props) {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (isBinaryMpp(headerBuf)) {
        // Binary .mpp — send to server-side parser
        setParsing(false);
        setServerParsing(true);
        try {
          parsed = await parseMppViaApi(file);
        } finally {
          setServerParsing(false);
        }

        // If server found no tasks but returned metadata, show the helpful error
        if (parsed.tasks.length === 0 && parsed.errors.length > 0) {
          setError(parsed.errors[0]);
          return;
        }
      } else {
        // Text-based file — parse client-side
        ...
        if (lowerName.endsWith(".xml")) {
          ...
          // Auto-detect
          parsed = parseMspXml(text);
          if (parsed.tasks.length === 0) parsed = parseSimpleCsv(text);
        }
  ...
  const toggleAll = () => {
    ...
    setSelectedTasks(
      selectedTasks.size === nonSummary.length
        ? new Set()
        : new Set(nonSummary.map((t) => t.uid))
    );
  };
  ...
  const isBusy = parsing || serverParsing;
      ...
      <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[85vh] flex flex-col shadow-2xl">
        ...
        <div className="p-5 border-b border-slate-200 flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              {step === "upload" &&
                (serverParsing
                  ? "Parsing .mpp on server…"
                  : "Upload an .mpp, .xml (MSPDI), or .csv file.")}
              {step === "preview" &&
                `${result?.tasks.length ?? 0} rows found — select tasks to import.`}
                ...
                {serverParsing ? (
                  <>
                    ...
                    <p className="font-semibold text-slate-700">
                      Sending to server for .mpp parsing…
                    </p>
                    <p className="text-sm text-slate-400 mt-1">
                      Extracting task data from binary format
                    </p>
                  </>
                ) : parsing ? (
                  <>
                  ...
                  <>
                    ...
                    <p className="text-xs text-slate-400 mt-3">
                      Supports .mpp · .xml (MSPDI) · .csv · .tsv
                    </p>
                  </>
                )}
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                  <div className="bg-white rounded-lg p-3 border border-blue-200 ring-1 ring-blue-100">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-bold text-blue-700">.mpp</span>
                      <span className="bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded text-[10px] font-bold">
                        NEW
                      </span>
                    </div>
                    <p className="text-slate-500">
                      Binary MS Project files. Parsed server-side — works for Project 2013+.
                    </p>
                  </div>
                  <div className="bg-white rounded-lg p-3 border border-amber-200 ring-1 ring-amber-100">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-bold text-amber-700">.xml (MSPDI)</span>
                      <span className="bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded text-[10px] font-bold">
                        RECOMMENDED
                      </span>
                    </div>
                    <p className="text-slate-500">
                      From MS Project: File → Save As → XML Format (.xml)
                    </p>
                  </div>
                  <div className="bg-white rounded-lg p-3 border border-slate-200">
                    <p className="font-bold text-slate-700 mb-1">.csv / .tsv</p>
                    <p className="text-slate-500">
                      Headers: Task Name, Duration, Start, Finish, % Complete
                    </p>
                  </div>
                </div>
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800">
                  <strong>💡 Tip for .mpp files:</strong> If task extraction fails (older Project
                  versions use a format we cannot decode), use File → Save As → XML Format in MS
                  Project, then upload the XML here instead.
                </div>
              </div>
              ...
              {result.projectName && (
                <div className="bg-slate-50 rounded-lg p-3 text-sm flex flex-wrap gap-4">
                  ...
                  <span className="text-slate-500">
                    {result.tasks.filter((t) => !t.summary).length} tasks,{" "}
                    {result.tasks.filter((t) => t.summary).length} groups
                  </span>
                </div>
              )}
                    ...
                    <tr>
                      <th className="p-2 w-8">
                        <input
                          type="checkbox"
                          checked={
                            result.tasks.filter((t) => !t.summary).length > 0 &&
                            selectedTasks.size ===
                              result.tasks.filter((t) => !t.summary).length
                          }
                          onChange={toggleAll}
                        />
                      </th>
                    ...
                    {result.tasks.map((task) => (
                      <tr
                        ...
                        className={`border-t border-slate-100 ${
                          task.summary ? "bg-slate-800 text-white" : ""
                        } ${
                      ...
                      >
                        ...
                        <td
                          className="p-2"
                          style={{ paddingLeft: `${8 + task.outlineLevel * 14}px` }}
                        >
                          {task.milestone && (
                        ...
                        <td className="p-2 text-xs font-mono text-slate-400 hidden md:table-cell">
                          {task.wbsCode ?? ""}
                        </td>
                        <td className="p-2 text-xs text-slate-500">
                          {task.durationDays != null ? `${task.durationDays}d` : "—"}
                        </td>
                        <td className="p-2 text-xs text-slate-500 hidden sm:table-cell">
                          {task.start ?? "—"}
                        </td>
                        <td className="p-2 text-xs text-slate-500 hidden sm:table-cell">
                          {task.finish ?? "—"}
          ...
          {step === "importing" && (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-400 text-sm mt-1">
                This may take a moment for large plans.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="p-4 border-t border-slate-200 flex items-center justify-between flex-shrink-0">
          {step === "preview" ? (
            <>
              ...
              <div className="flex items-center gap-3">
                <span className="text-sm text-slate-500">
                  ...
                  {result?.tasks.filter((t) => !t.summary).length ?? 0} tasks selected
                </span>
                <button
            ...
            <button
              onClick={onClose}
              className="px-4 py-2 rounded-lg text-sm text-slate-600 hover:bg-slate-100 transition-colors ml-auto"
            >
